...
public:
    ...
    template <class T>
    T get_value(const std::string currentSection, const std::string key)
    {
        static_assert(sizeof(T) == static_cast<int>(Warnings::dataTypeFalse), "Invalid data type");
    }

    template <>
    std::string get_value(const std::string currentSection, const std::string key)
    {
        return getStringValue(currentSection, key, lineNumber_);
    }

    template <>
    int get_value(const std::string currentSection, const std::string key)
    {
