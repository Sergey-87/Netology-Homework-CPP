...
DB::DB(const std::string& connection) : connection_{ connection } { std::cout << "Connected to database successfully!" << std::endl; };
...
void DB::addClient(const std::string& name, const std::string& surname, const std::string& email, const std::string& phone)
{
...
void DB::addPhone(const std::string& name, const std::string& phone)
{
    pqxx::transaction trn(connection_);
...
void DB::updateClient(const std::string& email, const std::string& newName, const std::string& newSurname, const std::string& newEmail)
{
    pqxx::transaction trn(connection_);
...
void DB::removeClient(const std::string& email)
{
    pqxx::transaction trn(connection_);
    pqxx::result res = trn.exec_params("SELECT id, name, surname FROM clients WHERE email = $1;", email);
    if (!res.empty()) {
...
std::vector<Client> DB::findClients(const std::string& query)
{
