...
Point Rectangle::fourthVertex(Point p1, Point p2, Point p3)
{
	...
	if ((p2.x - p1.x) * (p3.x - p1.x) + (p2.y - p1.y) * (p3.y - p1.y) == 0)
	{
		...
		std::cout << "Эти точки не могут быть вершинами прямоугольника.\n";
	}
	return p4;
}
std::vector<double> Rectangle::rectangleSides(Point p1, Point p2, Point p3) {
	double sideA{};
	double sideB{};
	double a = lengthOfSegment(p1, p2);
	double b = lengthOfSegment(p1, p3);
	double c = lengthOfSegment(p2, p3);
	if (a > b && a > c) {
		sideA = b;
		sideB = c;
	}
...
void Rectangle::printInfo() {
	...
	std::cout << std::endl << "сторона a = " << sides[0] << " сторона b = " << sides[1];
	std::cout << std::endl << "площадь = " << this->getSquare() << std::endl;
}